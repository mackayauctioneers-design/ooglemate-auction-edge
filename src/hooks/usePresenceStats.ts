...
export interface PresenceStats {
  ...
  newToday: number;
  seenThisRun: number;     // Listings touched by this run
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        // Get seen this run count (all listings touched by this run)
        const { count: seenCount } = await supabase
          .from('vehicle_listings')
          .select('*', { count: 'exact', head: true })
          .eq('last_ingest_run_id', latestRun.id);

        // Get still active count (dealer-grade, active status)
