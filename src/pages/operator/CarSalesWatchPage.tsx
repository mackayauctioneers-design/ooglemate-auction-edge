...
import { ChevronDown, ChevronRight, Eye, ExternalLink, Loader2, TrendingDown } from 'lucide-react';
        ...
        <CollapsibleContent>
          <CardContent className="pt-0">
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground py-4">No listings found with "{badgeFilter}" badge.</p>
            ) : (
                        ...
                        <td className="py-2 pr-4 text-right tabular-nums">
                          {l.price_difference_percent != null ? (
                            <span className="text-green-600 dark:text-green-400 font-semibold">
                              {l.price_difference_percent.toFixed(0)}%
                            </span>
                          ) : '—'}
                        </td>
...
export default function CarSalesWatchPage() {
  const [wellBelow, setWellBelow] = useState<RetailListing[]>([]);
  const [below, setBelow] = useState<RetailListing[]>([]);
  ...
  async function loadListings() {
    setIsLoading(true);
    try {
      const [wbRes, bRes] = await Promise.all([
        supabase
      ...
      if (wbRes.data) setWellBelow(wbRes.data as RetailListing[]);
      if (bRes.data) setBelow(bRes.data as RetailListing[]);
    } catch (err) {
  ...
  return (
    <OperatorLayout>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Active retail listings priced below market value — sorted by biggest delta.
            </p>
          </div>
        ...
        <PriceBadgeSection
          title="Well Below Market Value"
          badgeFilter="Well Below Market"
          color="destructive"
          listings={wellBelow}
          isLoading={isLoading}
          defaultOpen={true}
