-- Recreate sales_clearance_velocity with median_profit_dollars
...
SELECT
  ...
  percentile_cont(0.5) WITHIN GROUP (ORDER BY (sale_price - buy_price)::double precision)
    FILTER (WHERE sale_price IS NOT NULL AND buy_price IS NOT NULL)::integer AS median_profit_dollars
FROM vehicle_sales_truth
WHERE days_to_clear IS NOT NULL
GROUP BY account_id, make, model, variant;

-- Recreate sales_variation_performance with median_profit_dollars
