-- =====================================================
-- KITING MODE v2 - FINAL SPEC IMPLEMENTATION
-- Source Tier Priority + DNA-First Ranking
-- =====================================================

-- =====================================================
-- UPDATE rpc_get_unified_candidates with Source Tier Sorting
-- Sort: 1. dna_score DESC, 2. source_tier, 3. price ASC
-- =====================================================
...
BEGIN
  ...
  WITH ranked AS (
    SELECT
      ...
      -- SOURCE TIER: Tier 1 (auctions) = 1, Tier 2 (marketplaces) = 2, Tier 3 (dealers/internal) = 3
      CASE 
  ...
  FROM ranked r
  -- =====================================================
  -- FINAL SORT ORDER (per spec):
  -- 1. dna_score DESC (fingerprint accuracy first)
  -- 2. source_tier ASC (Tier 1 auction > Tier 2 marketplace > Tier 3 dealer)
  -- 3. asking_price ASC (cheapest last, only after relevance + tier)
  -- 4. km ASC (tiebreaker)
  -- =====================================================
  ORDER BY 
...
-- =====================================================
-- UPDATE rpc_get_unified_candidates_count to match criteria version filtering
-- =====================================================
...
-- Add source_tier column to hunt_unified_candidates for pre-computation
ALTER TABLE public.hunt_unified_candidates
ADD COLUMN IF NOT EXISTS source_tier INTEGER DEFAULT 3;

-- Create index for efficient sorting
CREATE INDEX IF NOT EXISTS idx_hunt_unified_dna_tier_price 
ON hunt_unified_candidates (hunt_id, criteria_version, is_stale, dna_score DESC, source_tier ASC, price ASC NULLS LAST);

COMMENT ON COLUMN hunt_unified_candidates.source_tier IS 'Source priority: 1=auction (Pickles/Manheim/Grays/Lloyds), 2=marketplace (Carsales/Autotrader/Drive/Gumtree), 3=dealer/internal';