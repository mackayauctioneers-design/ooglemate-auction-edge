...
SELECT
  ...
  vl.year, vl.km AS kilometres, vl.asking_price AS price,
  vl.fuel AS fuel_type, vl.transmission, vl.drivetrain,
  NULL::text AS body_type, NULL::text AS colour,
  vl.state, vl.postcode, vl.suburb, vl.location AS region_raw,
...
WHERE vl.make IS NOT NULL AND vl.model IS NOT NULL
  AND vl.last_seen_at > (now() - interval '14 days')
  AND COALESCE(vl.lifecycle_state, vl.status) NOT IN ('SOLD','DEAD','DELISTED','sold','dead','delisted')
...
SELECT
  ...
  rl.year, rl.km AS kilometres, rl.asking_price AS price,
  rl.fuel_type, rl.transmission, rl.drivetrain,
  rl.body_type, rl.colour,
  rl.state, rl.postcode, rl.suburb, rl.region_raw,
...
WHERE rl.make IS NOT NULL AND rl.model IS NOT NULL
  AND rl.last_seen_at > (now() - interval '14 days')
  AND COALESCE(rl.lifecycle_status, 'ACTIVE') NOT IN ('DELISTED','SOLD','DEAD');

CREATE INDEX IF NOT EXISTS idx_vl_fingerprint_hash ON public.vehicle_listings(fingerprint_hash) WHERE fingerprint_hash IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_rl_fingerprint_hash ON public.retail_listings(fingerprint_hash) WHERE fingerprint_hash IS NOT NULL;
