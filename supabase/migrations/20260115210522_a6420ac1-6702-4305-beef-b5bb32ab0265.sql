-- Fix upsert_retail_listing RPC to use correct column names
CREATE OR REPLACE FUNCTION public.upsert_retail_listing(
  ...
  p_seller_name_raw text DEFAULT NULL,
...
BEGIN
  ...
  IF v_existing_id IS NULL THEN
    ...
    INSERT INTO retail_listings (
      source, source_listing_id, make, model, variant_raw, year, km,
      asking_price, listing_url, state, postcode, suburb,
      seller_name_raw, seller_type,
    ...
    VALUES (
      p_source, p_source_listing_id, p_make, p_model, p_variant_raw, p_year, p_km,
      p_asking_price, p_listing_url, p_state, p_postcode, p_suburb,
      p_seller_name_raw, p_seller_type,
  ...
  ELSE
    ...
    SET
      ...
      seller_name_raw = COALESCE(p_seller_name_raw, seller_name_raw),
