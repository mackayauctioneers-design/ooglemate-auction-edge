-- Fix: Ensure canonical_id is never null in hunt_unified_candidates
-- Use source_url md5 hash as fallback when canonical_id extraction fails
...
BEGIN
  ...
  SELECT
    ...
    hec.source_name,
    hec.source_url,
    -- GUARANTEED non-null canonical_id: use extracted or fallback to md5(url)
    COALESCE(
