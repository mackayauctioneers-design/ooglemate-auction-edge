...
-- Simplified evaluate_watch_status without attempt_count for now
...
DECLARE
  ...
  v_first_seen_at timestamptz; v_asking_price numeric;
  v_sold_returned_suspected boolean; v_sold_returned_reason text;
  v_fingerprint RECORD; v_outcome RECORD;
...
BEGIN
  ...
  INTO v_make, v_model, v_year, v_source_class, v_first_seen_at, v_asking_price,
       v_sold_returned_suspected, v_sold_returned_reason, v_days_on_market
  ...
  IF FOUND THEN
    ...
    IF v_confidence IN ('med', 'high') AND v_source_class = 'classifieds' AND v_days_on_market >= 60 THEN
      v_status := 'buy_window';
