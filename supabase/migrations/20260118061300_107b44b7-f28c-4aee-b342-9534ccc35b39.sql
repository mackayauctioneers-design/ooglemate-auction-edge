...
-- Fix rpc_build_unified_candidates to use correct retail_listings column names
...
BEGIN
  ...
  -- Insert INTERNAL candidates (using correct column names: source, match_score from somewhere)
  INSERT INTO hunt_unified_candidates (
    hunt_id, criteria_version, source, source_tier, url, title, variant_raw,
    make, model, year, km, price, location,
  ...
  SELECT 
    ...
    END,
    rl.listing_url,
    NULL, -- no title column in retail_listings
    rl.variant_raw,
    rl.make,
    rl.model,
    rl.year,
    ...
    'UNKNOWN', -- engine_family not in retail_listings
    COALESCE(rl.body_type, 'UNKNOWN'),
    COALESCE(rl.cab_type, 'UNKNOWN'),
    COALESCE(rl.badge, 'UNKNOWN'),
    fn_build_identity_key(rl.make, rl.model, rl.series_family, rl.badge, rl.body_type, rl.cab_type, NULL),
    0.7, -- internal listings have higher base confidence
    '{}'::jsonb,
    'listing', -- internal retail listings are always listings
    'INTERNAL_LISTING',
    true,
    7.0, -- internal listings get base score
    7.0,
    ARRAY['INTERNAL'],
    CASE
      ...
      WHEN rl.asking_price IS NOT NULL THEN 'BUY'
      ELSE 'WATCH'
    END
  ...
  -- Insert OUTWARD candidates with proper DNA scoring
  INSERT INTO hunt_unified_candidates (
    hunt_id, criteria_version, source, source_tier, url, title, variant_raw,
    make, model, year, km, price, location,
    series_family, engine_family, body_type, cab_type, badge,
