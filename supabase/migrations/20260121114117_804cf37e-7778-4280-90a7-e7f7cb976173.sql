-- Update fn_build_retail_geo_heat_sa2_daily with state fallback from parsed location
...
BEGIN
  ...
  WITH listing_base AS (
    SELECT
      vl.sa2_code,
      COALESCE(vl.state, (SELECT p.state FROM fn_parse_location_au(vl.location) p LIMIT 1), gs.state) AS state,
      vl.make,
      vl.model_family,
      vl.first_seen_at,
      vl.delisted_at,
