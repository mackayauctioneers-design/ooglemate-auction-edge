-- Update upsert_retail_listing to accept and store price_type
CREATE OR REPLACE FUNCTION public.upsert_retail_listing(
  ...
  p_price_type text DEFAULT 'unknown'::text
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_existing_id IS NULL THEN
    ...
    INSERT INTO retail_listings (
      ...
      km, asking_price, state, suburb, price_type,
    ...
    VALUES (
      ...
      p_km, p_asking_price, p_state, p_suburb, COALESCE(p_price_type, 'unknown'),
  ...
  ELSE
    ...
    SET
      ...
      -- Only upgrade price_type, never downgrade to unknown
      price_type = CASE 
        WHEN p_price_type IS NOT NULL AND p_price_type != 'unknown' THEN p_price_type
        ELSE COALESCE(price_type, 'unknown')
      END,
      last_seen_at = now(),
  ...
  RETURN jsonb_build_object(
    ...
    'sold_returned', v_sold_returned_triggered
  );
END;
$function$;