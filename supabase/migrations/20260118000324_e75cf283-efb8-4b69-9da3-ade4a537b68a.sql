-- Update RPC to support source_type filtering and return verification status
...
CREATE OR REPLACE FUNCTION rpc_get_unified_candidates(
  ...
  p_source_filter text DEFAULT NULL -- 'outward' or 'internal' or null for all
)
RETURNS TABLE (
  id uuid,
  source_type text,
  ...
  is_verified boolean -- NEW: whether price was verified via scrape
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  WITH ranked AS (
    SELECT 
      huc.*,
      -- Check if this is a verified outward candidate
      CASE 
        WHEN huc.source_type = 'outward' THEN 
          EXISTS (
            SELECT 1 FROM hunt_external_candidates hec 
            WHERE hec.id::text = huc.source_listing_id 
              AND hec.price_verified = true
          )
