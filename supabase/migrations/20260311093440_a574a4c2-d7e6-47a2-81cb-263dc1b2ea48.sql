...
ALTER TABLE operator_opportunities ADD COLUMN IF NOT EXISTS suppress_anchor boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION upsert_operator_opportunity_guarded(p_row jsonb)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT status, COALESCE(dismissed_anchor_ids, '{}'), COALESCE(suppress_anchor, false)
  INTO v_existing_status, v_dismissed, v_suppress
  ...
  -- If suppress_anchor is true OR the incoming anchor is in the dismissed list, null out anchor fields
  IF v_suppress OR (v_new_anchor IS NOT NULL AND v_dismissed IS NOT NULL AND v_new_anchor = ANY(v_dismissed)) THEN
    ...
    ON CONFLICT (listing_id) DO UPDATE SET
      ...
      -- Do NOT overwrite anchor fields — they stay null because anchor was suppressed/dismissed
