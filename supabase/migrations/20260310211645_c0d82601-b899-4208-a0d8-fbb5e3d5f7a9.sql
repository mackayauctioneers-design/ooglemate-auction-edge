...
-- RPC to snapshot active listings from market_listings into model_market_snapshot
...
BEGIN
  INSERT INTO public.model_market_snapshot (make, model, variant_resolved, region, observed_at, active_listing_count, avg_price, avg_km, avg_days_on_market)
  SELECT
    make,
    model,
    COALESCE(variant, 'UNKNOWN'),
  ...
  GROUP BY make, model, COALESCE(variant, 'UNKNOWN'), COALESCE(location, 'UNKNOWN')
