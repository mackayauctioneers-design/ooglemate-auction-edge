...
-- Fix rpc_build_unified_candidates to use correct column names from hunt_external_candidates
-- The columns are: listing_intent, listing_intent_reason, series_family, engine_family, body_type, cab_type, badge, identity_confidence
-- NOT: ext_listing_intent, ext_listing_intent_reason, ext_series_family, etc.
...
BEGIN
  ...
  outward_classified AS (
    SELECT
      oraw.*,
      -- Use actual column names: listing_intent, listing_intent_reason (not ext_ prefixed)
      CASE
        WHEN oraw.listing_intent IS NOT NULL AND oraw.listing_intent != 'unknown'
        THEN oraw.listing_intent
        ELSE (oraw.intent_result)->>'intent'
      END AS computed_intent,
      CASE
        WHEN oraw.listing_intent IS NOT NULL AND oraw.listing_intent != 'unknown'
        THEN oraw.listing_intent_reason
        ELSE (oraw.intent_result)->>'reason'
      END AS computed_intent_reason,

      -- Use actual column names: series_family, engine_family, body_type, cab_type, badge (not ext_ prefixed)
      NULLIF(COALESCE(oraw.series_family, (oraw.ident_result)->>'series_family'), 'UNKNOWN') AS computed_series,
      NULLIF(COALESCE(oraw.engine_family, (oraw.ident_result)->>'engine_family'), 'UNKNOWN') AS computed_engine,
      NULLIF(COALESCE(oraw.body_type, (oraw.ident_result)->>'body_type'), 'UNKNOWN') AS computed_body,
      NULLIF(COALESCE(oraw.cab_type, (oraw.ident_result)->>'cab_type'), 'UNKNOWN') AS computed_cab,
      NULLIF(COALESCE(oraw.badge, (oraw.ident_result)->>'badge'), 'UNKNOWN') AS computed_badge,

      -- Use actual column name: identity_confidence (not ext_identity_confidence)
      COALESCE(oraw.identity_confidence, ((oraw.ident_result)->>'confidence')::NUMERIC) AS computed_identity_conf
  ...
  WITH internal_raw AS (
    ...
    WHERE vl.status = 'active'
      ...
      AND (v_hunt.year IS NULL OR vl.year BETWEEN v_hunt.year - 2 AND v_hunt.year + 2)
    LIMIT 500
  ),
