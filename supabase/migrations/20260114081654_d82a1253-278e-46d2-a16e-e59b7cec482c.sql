-- Drop and recreate compute_proven_exit with fixed column references
...
BEGIN
  ...
  SELECT
    ...
    MAX(se.exit_date),
    MIN(se.exit_date),
    EXTRACT(DAY FROM (CURRENT_DATE - MAX(se.exit_date)))::INTEGER,
    ARRAY_AGG(DISTINCT se.source_type),
    ARRAY_AGG(DISTINCT se.dealer_id) FILTER (WHERE se.dealer_id IS NOT NULL)
  INTO v_exit_value, v_sample_size, v_newest_date, v_oldest_date, v_recency_days, v_sources, v_dealers
