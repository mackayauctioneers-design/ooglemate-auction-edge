...
-- 1. Scorer cursor table for delta tracking
CREATE TABLE IF NOT EXISTS public.scorer_cursors (
  job_name text PRIMARY KEY,
  last_seen_cutoff timestamptz,
  last_run_at timestamptz,
  last_ok boolean DEFAULT true,
  note jsonb DEFAULT '{}'::jsonb
);

ALTER TABLE public.scorer_cursors ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin only scorer_cursors" ON public.scorer_cursors
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin')
  );

-- Seed the cursor 48h back so first run picks up recent listings
INSERT INTO public.scorer_cursors (job_name, last_seen_cutoff, last_run_at)
VALUES ('operator_scorer_v2', now() - interval '48 hours', now())
ON CONFLICT (job_name) DO NOTHING;

-- 2. Guarded upsert function: respects terminal states
CREATE OR REPLACE FUNCTION public.upsert_operator_opportunity_guarded(
  p_row jsonb
) RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Upsert (insert or update actionable)
  ...
  VALUES (
    ...
    CASE WHEN p_row->>'anchor_sale_id' IS NOT NULL THEN (p_row->>'anchor_sale_id')::uuid ELSE NULL END,
