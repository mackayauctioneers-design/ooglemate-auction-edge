-- ============================================================
-- FIX: Bulletproof division + remove pointless rounding
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- Calculate gaps: NULLIF for bulletproof division, no rounding on dollars
  -- ============================================================
  v_gap_dollars := v_proven.exit_value::NUMERIC - v_listing.asking_price::NUMERIC;
  v_gap_pct := (v_gap_dollars / NULLIF(v_proven.exit_value::NUMERIC, 0)) * 100.0;
  ...
  -- Confidence gate: ordered ranking
  ...
  v_required_rank := CASE v_config_confidence
    WHEN 'high' THEN 3 WHEN 'medium' THEN 2 WHEN 'low' THEN 1 ELSE 1
  END;
  ...
  IF v_config.guardrail_type = 'layered' THEN
    IF v_gap_pct >= v_pct_threshold AND v_gap_dollars >= v_abs_threshold THEN
      ...
      END IF;
    ELSE
      IF v_gap_dollars > 0 AND v_watch_gates_pass THEN
        v_result := 'WATCH';
        v_reasons := v_reasons || 'below_buy_threshold';
      ELSE
        v_result := 'IGNORE';
        IF NOT v_watch_gates_pass THEN v_reasons := v_reasons || 'watch_gates_failed'; END IF;
      END IF;
    END IF;
...
-- Remove the broken pg_cron job
SELECT cron.unschedule('trigger-backfill-nightly');