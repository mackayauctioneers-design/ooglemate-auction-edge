-- Fix the rpc_get_unified_candidates to check asking_price in extracted
-- The outward_candidates store price as extracted->>'asking_price'
...
BEGIN
  ...
  WITH ranked AS (
    SELECT 
      ...
      -- For outward candidates, check if price/asking_price was extracted
      -- For internal candidates, they are always considered verified
      CASE 
        WHEN huc.source_type = 'outward' THEN 
          COALESCE(
            -- Check outward_candidates: price is stored as 'asking_price' in extracted
