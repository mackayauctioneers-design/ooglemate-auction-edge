-- Fix derive_presence_events_v2:
-- 1. BUG FIX: prev_status was capturing NEW status, not old status
--    Solution: Use pre-select CTE to capture old status before update
-- 2. Circuit breaker is now meaningful per-source (caller should pass p_source='pickles')
...
BEGIN
  ...
  -- This is SOURCE-SPECIFIC when p_source is provided
  ...
  -- Count how many were seen (touched) in this run - SOURCE-SPECIFIC
  ...
  -- FIX: Pre-select to capture ACTUAL previous status before update
  WITH to_clear AS (
    SELECT id, source, status AS prev_status
    FROM vehicle_listings
    WHERE (p_source IS NULL OR source = p_source)
      AND status IN ('catalogue', 'listed', 'active')
      AND is_dealer_grade = true
      AND missing_streak >= 2
  ),
  gone AS (
    UPDATE vehicle_listings v
    SET 
    ...
    FROM to_clear t
    WHERE v.id = t.id
    RETURNING v.id, v.source, t.prev_status
  ),
  logged AS (
    ...
    SELECT 
      ...
      p_run_id, 
      g.source,
      g.prev_status,  -- Now correctly captures old status
      'cleared', 
