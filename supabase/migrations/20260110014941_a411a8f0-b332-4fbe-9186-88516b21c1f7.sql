...
-- Fix trap_deals to use fingerprints that have prices (active listings)
-- Clearances are for TTD only; prices come from current listings
...
WITH fp AS (
  ...
  WHERE avg_price IS NOT NULL 
    AND avg_price > 0
    AND listing_total >= 1  -- At least 1 listing to have a price benchmark
),
...
fp_national AS (
  SELECT 
    ...
    year_min,
    year_max,
    SUM(listing_total) as listing_total,
...
COMMENT ON VIEW trap_deals IS 'Dealer retail inventory with tiered fingerprint matching. Uses listing_total for sample size and avg_price for benchmarks. T1-T4=same region with decreasing specificity. T5=national fallback.';
