-- Drop and recreate evaluate_trigger with fixes
...
BEGIN
  ...
  WHERE tc.version = p_config_version AND tc.is_active = true;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Config not found or inactive: %', p_config_version;
  END IF;

  -- Get proven exit
  SELECT * INTO v_proven
  FROM proven_exits pe
  WHERE pe.identity_id = v_listing.identity_id;

  -- Thresholds from config
  v_pct_threshold := v_config.guardrail_pct;
  v_abs_threshold := v_config.guardrail_abs;
  v_max_gap_threshold := COALESCE(v_config.guardrail_max_gap, 15000);
  
  -- Confidence ranking (high=3, medium=2, low=1)
  v_min_confidence_rank := CASE v_config.min_confidence_buy
    WHEN 'high' THEN 3
    WHEN 'medium' THEN 2
    WHEN 'low' THEN 1
    ELSE 1
  ...
  IF v_proven IS NULL THEN
    v_result := 'IGNORE';
    v_gate_failures := array_append(v_gate_failures, 'no_proven_exit');
    ...
    VALUES (p_listing_id, v_listing.source, p_config_version, v_listing.identity_id, v_listing.asking_price, v_result, v_reasons, v_gate_failures,
      jsonb_build_object('listing', row_to_json(v_listing), 'config', row_to_json(v_config)))
    RETURNING id INTO v_evaluation_id;

    RETURN QUERY SELECT v_evaluation_id, v_result, NULL::NUMERIC, NULL::NUMERIC, v_reasons, v_gate_failures;
    RETURN;
  END IF;
  ...
  IF v_gap_dollars <= 0 THEN
    v_result := 'IGNORE';
    v_reasons := array_append(v_reasons, 'gap_not_positive');
    ...
    VALUES (p_listing_id, v_listing.source, p_config_version, v_listing.identity_id, v_listing.asking_price, v_proven.exit_value, v_gap_dollars, v_gap_pct, v_result, v_reasons, v_gate_failures,
  ...
  v_proven_confidence_rank := CASE v_proven.confidence_label
    WHEN 'high' THEN 3
    WHEN 'medium' THEN 2
    WHEN 'low' THEN 1
    ELSE 0
  ...
  IF v_proven_confidence_rank < v_min_confidence_rank THEN
    v_result := 'IGNORE';
    v_gate_failures := array_append(v_gate_failures, 'confidence_too_low');
  ...
  -- Check recency gate (270 days max)
  IF v_proven.sale_recency_days > 270 THEN
    v_result := 'IGNORE';
    v_gate_failures := array_append(v_gate_failures, 'stale_evidence');
  ...
  -- Check max gap guardrail
  IF v_gap_dollars > v_max_gap_threshold THEN
    v_result := 'WATCH';
    v_reasons := array_append(v_reasons, 'exceeds_max_gap');
  ...
  -- Check BUY thresholds: gap >= pct% AND gap >= $abs
  IF v_gap_pct >= v_pct_threshold AND v_gap_dollars >= v_abs_threshold THEN
    v_result := 'BUY';
    v_reasons := array_append(v_reasons, 'pct_threshold_met');
    v_reasons := array_append(v_reasons, 'abs_threshold_met');
  ELSE
    v_result := 'WATCH';
    v_reasons := array_append(v_reasons, 'below_buy_threshold');
  END IF;
  ...
  -- Emit trigger (WATCH or BUY only)
  INSERT INTO sales_triggers (listing_id, trigger_type, asking_price, proven_exit_value, gap_dollars, gap_pct, identity_id, config_version)
