-- FINAL FIX: Remove outward pre-filter + Fix internal tier logic
...
BEGIN
  ...
  -- FIX: Proper tier logic - auctions=1, marketplaces=2, dealer=3
  WITH internal_candidates AS (
    SELECT DISTINCT ON (rl.id)
      ...
      CASE 
        WHEN rl.source IN ('pickles','manheim','grays','lloyds','lloydsauctions','graysonline') 
             OR rl.source ILIKE '%auction%' THEN 1
        WHEN rl.source IN ('carsales','autotrader','drive','gumtree','gumtree_dealer') 
             OR rl.source ILIKE '%carsales%' OR rl.source ILIKE '%autotrader%' 
             OR rl.source ILIKE '%gumtree%' THEN 2
        ELSE 3 
      END AS source_tier,
  ...
  SELECT ic.hunt_id, ic.criteria_version, ic.source_type, ic.source_val, ic.source_tier, ic.url, ic.asking_price, ic.km, ic.year,
    ...
    CASE WHEN v_hunt.required_series_family IS NOT NULL AND (ic.identity_result->>'series_family') != 'UNKNOWN' 
         ...
         ELSE ARRAY[]::TEXT[] END,
    ic.verified, (100 - ic.source_tier * 10) + COALESCE(ic.match_score, 0), 
    ARRAY['TIER_' || ic.source_tier, 'INTERNAL']
  FROM internal_candidates ic;
  GET DIAGNOSTICS v_inserted_internal = ROW_COUNT;

  -- Outward candidates - FIX: NO pre-filter by decision, let classifier decide
  -- Filter by criteria_version and is_stale instead
  WITH outward_classified AS (
    SELECT hec.id, hec.hunt_id, hec.source_url AS url, hec.title, hec.raw_snippet AS snippet,
      ...
      -- Classify intent (this decides IGNORE for non-listings)
      fn_classify_listing_intent(hec.source_url, hec.title, hec.raw_snippet) AS intent_result,
      -- Classify identity (this decides series mismatch)
      fn_classify_vehicle_identity(hec.make, hec.model, hec.variant_raw, hec.source_url, 
        hec.title || ' ' || COALESCE(hec.raw_snippet, '')) AS identity_result
  ...
  SELECT oc.hunt_id, v_criteria_version, 'outward'::TEXT, oc.source_val,
    -- Proper tier for outward too
    CASE 
      WHEN oc.url ILIKE '%pickles%' OR oc.url ILIKE '%manheim%' OR oc.url ILIKE '%lloyds%' OR oc.url ILIKE '%grays%' THEN 1 
      WHEN oc.url ILIKE '%autotrader%' OR oc.url ILIKE '%carsales%' OR oc.url ILIKE '%gumtree%' OR oc.url ILIKE '%drive.com%' THEN 2 
      ELSE 3 
    END,
    ...
    -- PROOF GATING DECISION (classifier decides, not pre-filter)
    CASE 
      ...
      WHEN v_hunt.required_series_family IS NOT NULL 
           AND (oc.identity_result->>'series_family') = 'UNKNOWN' THEN 'UNVERIFIED'
    ...
    -- REASONS
    CASE 
      WHEN (oc.intent_result->>'intent') = 'non_listing' THEN ARRAY[(oc.intent_result->>'reason')]
      WHEN v_hunt.required_series_family IS NOT NULL 
           AND (oc.identity_result->>'series_family') != 'UNKNOWN' 
      ...
      WHEN v_hunt.required_series_family IS NOT NULL 
           AND (oc.identity_result->>'series_family') = 'UNKNOWN' THEN ARRAY['SERIES_UNKNOWN']
      ELSE ARRAY[]::TEXT[] 
    END,
    COALESCE(oc.was_verified, false), 
    CASE WHEN (oc.intent_result->>'intent') = 'non_listing' THEN 0 ELSE 75 END,
    ARRAY['INTENT_' || (oc.intent_result->>'intent'), 'SERIES_' || (oc.identity_result->>'series_family'), 'OUTWARD']
  ...
  -- Update rank positions: BUY first, then WATCH, then UNVERIFIED, then IGNORE
  -- Within each: tier ASC (auctions first), rank_score DESC, price ASC
  WITH ranked AS (
