-- Fix rpc_build_unified_candidates to call fn_build_identity_key correctly
-- It takes 7 args: make, model, series, badge, body, cab, engine (from classification result)
...
BEGIN
  ...
  outward_classified AS (
    SELECT
      oraw.*,
      -- Extract fields from classification results
  ...
  outward_with_key AS (
    SELECT
      oc.*,
      -- Build identity key from classified fields
  ...
  outward_scored AS (
    SELECT
      ...
      CASE
        ...
        WHEN ow.computed_intent != 'listing' THEN 'IGNORE'
      ...
      CASE
        WHEN ow.computed_intent != 'listing' THEN 'NOT_LISTING'
        WHEN v_hunt.required_series_family IS NOT NULL 
             AND ow.computed_series IS NOT NULL 
  ...
  internal_scored AS (
    SELECT
      ...
      fn_compute_outward_dna_score(
        ...
        ib.computed_badge,
        ib.year,
        COALESCE(ib.title, '') || ' ' || COALESCE(ib.description, ''),
        COALESCE(v_hunt.required_series_family, v_hunt.series_family),
        COALESCE(v_hunt.required_engine_family, v_hunt.engine_family),
        COALESCE(v_hunt.required_body_type, v_hunt.body_type),
        COALESCE(v_hunt.required_badge, v_hunt.badge),
