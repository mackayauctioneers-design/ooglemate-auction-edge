-- Update claim_next_job to include stale-processing reaper
...
BEGIN
  ...
  -- Re-queue if attempts < max_attempts, else mark failed
  ...
  SET 
    status = 'processing',
    started_at = now(),
    finished_at = NULL,  -- Clear any previous finished_at
...
COMMENT ON FUNCTION public.claim_next_job IS 'Atomically claims next pending job. Also reaps stale jobs stuck in processing > 15 min.';