-- Add is_historical_result flag to vehicle_listings
ALTER TABLE public.vehicle_listings 
  ADD COLUMN IF NOT EXISTS is_historical_result boolean NOT NULL DEFAULT false;

-- Add indexes for lifecycle sweep performance
CREATE INDEX IF NOT EXISTS idx_vl_lifecycle_last_seen 
  ON public.vehicle_listings (lifecycle_state, last_seen_at);
CREATE INDEX IF NOT EXISTS idx_rl_lifecycle_last_seen 
  ON public.retail_listings (lifecycle_status, last_seen_at);

-- Recreate market_listings view with is_historical_result
