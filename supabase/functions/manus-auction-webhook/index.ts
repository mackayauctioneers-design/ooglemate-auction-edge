...
Deno.serve(async (req) => {
  ...
  // Only process auction detail enrichment tasks — route search tasks to manus-webhook logic
  if (filters.flow !== "auction_detail_enrichment") {
    console.log(`[AUCTION-WEBHOOK] Not an auction enrichment task — routing as search result: ${taskId}`);
    return await handleSearchTaskResult(supabase, task, taskId, resultMessage);
  }
...
/**
 * Handle non-auction search task results (OogleBot / Hunt flow).
 * Parses listings from Manus task_detail.message and writes to manus_search_tasks.results
 * so the frontend polling picks them up.
 */
async function handleSearchTaskResult(
  supabase: any,
  task: any,
  taskId: string,
  resultMessage: string,
): Promise<Response> {
  // Parse listings JSON array from the Manus result message
  let listings: any[] = [];
  try {
    const jsonMatch = resultMessage.match(/\[[\s\S]*\]/);
    if (jsonMatch) {
    ...
    console.error(`[AUCTION-WEBHOOK→SEARCH] Failed to parse listings JSON for ${taskId}:`, e);
  }

  console.log(`[AUCTION-WEBHOOK→SEARCH] Task ${taskId}: parsed ${listings.length} listings`);
  ...
  if (task.hunt_id) {
    for (const listing of listings) {
      ...
      if (!error) inserted++;
      else console.error(`[AUCTION-WEBHOOK→SEARCH] Hunt candidate insert error:`, error.message);
    }
    ...
    if (inserted > 0) {
      try {
        ...
        console.error(`[AUCTION-WEBHOOK→SEARCH] Failed to rebuild candidates:`, e);
      }
    }
  }
  // ── Route 2: OogleBot flow → retail_listings ──
    ...
    for (const listing of listings) {
      ...
      if (!error) inserted++;
      else console.error(`[AUCTION-WEBHOOK→SEARCH] Retail insert error:`, error.message);
    }
  }
  ...
  console.log(
    `[AUCTION-WEBHOOK→SEARCH] Done: ${inserted}/${listings.length} listings for ${task.hunt_id ? "hunt " + task.hunt_id : "session " + task.search_session_id}`,
  );

  return new Response(
    JSON.stringify({ ok: true, inserted, route: task.hunt_id ? "hunt" : "ooglebot" }),
    { status: 200, headers: { "Content-Type": "application/json" } },
  );
}
