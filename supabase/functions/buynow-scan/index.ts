...
Deno.serve(async (req) => {
  ...
  const { data } = await sb.from("dealer_liquidity_profiles").select("id").limit(1);
  return new Response(JSON.stringify({ ok: true, count: (data || []).length, m: fmtMoney(5000), h: h.substring(0, 8) }), { headers: { ...cors, "Content-Type": "application/json" } });
});
