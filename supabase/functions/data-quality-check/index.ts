...
const CHECKS: QualityCheck[] = [
  ...
  },
  {
    ...
    source: null,
    severity: "info",
    query: `SELECT ROUND(100.0 * COUNT(*) FILTER (WHERE variant_raw IS NULL AND variant_resolved IS NULL) / NULLIF(COUNT(*), 0), 1) as metric FROM market_listings WHERE last_seen_at > now() - interval '14 days'`,
    threshold: 30,
    direction: "above",
    description: "% of active listings with no variant data",
  },
  {
    name: "make_model_unresolved",
    source: null,
    severity: "critical",
    query: `SELECT ROUND(100.0 * COUNT(*) FILTER (WHERE make IS NULL OR model IS NULL) / NULLIF(COUNT(*), 0), 1) as metric FROM market_listings WHERE last_seen_at > now() - interval '7 days'`,
    threshold: 5,
    direction: "above",
    description: "% of recent listings with NULL make or model",
  },
  {
    name: "sudden_drop_carsales",
    source: "carsales",
    severity: "critical",
    query: `WITH current_count AS (SELECT COUNT(*) as cnt FROM retail_listings WHERE source = 'carsales' AND last_seen_at > now() - interval '24 hours'), prev_count AS (SELECT COUNT(*) as cnt FROM retail_listings WHERE source = 'carsales' AND last_seen_at > now() - interval '48 hours' AND last_seen_at <= now() - interval '24 hours') SELECT CASE WHEN prev_count.cnt = 0 THEN 0 ELSE ROUND(100.0 * (prev_count.cnt - current_count.cnt) / prev_count.cnt, 1) END as metric FROM current_count, prev_count`,
    threshold: 50,
    direction: "above",
    description: "% drop in Carsales listings vs previous 24h (>50% = scraper broken)",
  },
  {
    name: "sudden_drop_autotrader",
    source: "autotrader",
    severity: "critical",
    query: `WITH current_count AS (SELECT COUNT(*) as cnt FROM retail_listings WHERE source = 'autotrader' AND last_seen_at > now() - interval '24 hours'), prev_count AS (SELECT COUNT(*) as cnt FROM retail_listings WHERE source = 'autotrader' AND last_seen_at > now() - interval '48 hours' AND last_seen_at <= now() - interval '24 hours') SELECT CASE WHEN prev_count.cnt = 0 THEN 0 ELSE ROUND(100.0 * (prev_count.cnt - current_count.cnt) / prev_count.cnt, 1) END as metric FROM current_count, prev_count`,
    threshold: 50,
    direction: "above",
    description: "% drop in Autotrader listings vs previous 24h",
  },
];
