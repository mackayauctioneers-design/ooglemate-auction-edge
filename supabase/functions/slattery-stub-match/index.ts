...
/**
 * SLATTERY STUB MATCH - Lane 2: Match stubs to hunts and queue for deep-fetch
 * 
 * Production hardened (following Pickles/Manheim/Grays pattern):
 ...
 * - source='slattery' filter throughout
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[SLATTERY-MATCH] Starting stub matching: batch=${batch_size}, min_score=${min_match_score}`);
    ...
    // Fetch pending Slattery stubs (source='slattery')
    ...
    if (stubError || !stubs || stubs.length === 0) {
      console.log("[SLATTERY-MATCH] No pending Slattery stubs found");
      return new Response(
        JSON.stringify({ success: true, message: "No Slattery stubs to match", metrics }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    console.log(`[SLATTERY-MATCH] Found ${stubs.length} pending Slattery stubs`);
    ...
    if (specError || !specs || specs.length === 0) {
      console.log("[SLATTERY-MATCH] No enabled dealer specs found");
      return new Response(
    ...
    console.log(`[SLATTERY-MATCH] Found ${specs.length} enabled dealer specs`);
    ...
    console.log(`[SLATTERY-MATCH] Found ${matchResults.length} match pairs`);
    ...
    if (!dry_run && matchesByStub.size > 0) {
      ...
      if (queueError) {
        console.error("[SLATTERY-MATCH] Queue error:", queueError);
        metrics.errors.push(`Queue error: ${queueError.message}`);
      } else {
        metrics.queue_items_created = upserted?.length || 0;
        console.log(`[SLATTERY-MATCH] Queued ${metrics.queue_items_created} items`);
      }
      ...
      for (const stub of matchedStubs) {
        ...
        if (updateError) {
          console.error(`[SLATTERY-MATCH] Failed to update stub ${stub.id}:`, updateError.message);
    ...
    console.log(`[SLATTERY-MATCH] Completed in ${duration}ms:`, metrics);
    ...
    console.error("[SLATTERY-MATCH] Error:", error);
    return new Response(
      JSON.stringify({
        success: false,
