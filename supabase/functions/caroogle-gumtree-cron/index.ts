...
/**
 * CAROOGLE → GUMTREE FEED
 * 
 * Fetches Gumtree vehicle listings from Caroogle API
 * and upserts into vehicle_listings with source = "gumtree".
 * 
...
function inferSellerType(ad: Record<string, unknown>): string {
  const sellerType = String(ad.sellerType || ad.seller_type || "").toLowerCase();
  if (sellerType.includes("dealer")) return "dealer";
  if (sellerType.includes("private")) return "private";
  // Gumtree mixes private and dealer — default to private if unknown
  return "private";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${CRON_NAME}] Fetching from Caroogle API (source=gumtree, pageSize=${PAGE_SIZE})...`);
    ...
    console.log(`[${CRON_NAME}] Received ${ads.length} total Gumtree records across ${currentPage} page(s)`);

    if (ads.length === 0) {
      // Gumtree may have 0 records initially — log but don't fatal
      console.warn(`[${CRON_NAME}] Caroogle Gumtree API returned 0 records`);
    ...
    for (const ad of ads) {
      ...
      rows.push({
        ...
        status: "listed",
        seller_type: inferSellerType(ad),
