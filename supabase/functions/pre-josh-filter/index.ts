...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: rows, error: fetchErr } = await supabase
      ...
      .select(
        "id, listing_id, make, model, year, km, price, market_price, discount_pct, deal_score, deal_tag, price_badge, source, source_type, listing_url, flag_damage, flag_km_issue, flag_sold, flag_wrong_variant, condition_notes, seller_type, detected_at"
      )
      .eq("status", "NEW")
    ...
    if (!rows || rows.length === 0) {
      return new Response(
        JSON.stringify({ success: true, processed: 0, auto_rejected: 0, pre_approved: 0, kept: 0 }),
    ...
    await supabase.from("cron_heartbeat").upsert(
      {
        ...
        last_ok: true,
        note: `processed=${rows.length} rejected=${rejected.length} pre_approved=${approved.length} kept=${kept.length} ms=${elapsed}`,
      },
      { onConflict: "cron_name" }
    );

    console.log(
      `[PRE-JOSH] Done in ${elapsed}ms — rejected=${rejected.length}, pre_approved=${approved.length}, kept=${kept.length}`
    );
