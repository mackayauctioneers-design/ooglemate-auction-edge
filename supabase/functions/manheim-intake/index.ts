/**
 * manheim-intake — Secure push endpoint for the local Manheim scraper.
 *
 * Auth: Bearer ${MANHEIM_INGEST_KEY} (or MANUS_WEBHOOK_SECRET fallback).
 * Maps generic Manheim JSON → vehicle_listings (source='manheim', source_class='auction')
...
Deno.serve(async (req) => {
  ...
  const valid = [
    Deno.env.get("MANHEIM_INGEST_KEY"),
    Deno.env.get("MANUS_WEBHOOK_SECRET"),
  ].filter(Boolean) as string[];
  if (!valid.length || !valid.includes(token)) {
  ...
  for (const item of listings) {
    try {
      ...
      if (!stock || !make || !model || !Number.isFinite(year)) {
        skipped++; errors.push(`bad row: ${JSON.stringify({ stock, make, model, year })}`); continue;
      }

      const listingId = `manheim:${stock}`;
      ...
      if (error) { errors.push(`${listingId}: ${error.message}`); skipped++; continue; }
      upserted++;
      ...
      skipped++; errors.push(`item error: ${e?.message || e}`);
    }
  }

  console.log(`[manheim-intake] received=${listings.length} upserted=${upserted} skipped=${skipped}`);
  return json({ status: "ok", received: listings.length, upserted, skipped, errors: errors.slice(0, 20) });
});
